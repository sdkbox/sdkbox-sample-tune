...
public:
    ...
    virtual void onMobileAppTrackerDidReceiveDeeplink(const std::string &deeplink, bool timeout)
    {
        ...
        stack->pushBoolean(timeout);
        stack->executeFunctionByHandler(m_luaHandler, 3);
    }
    virtual void onMobileAppTrackerDidFailDeeplinkWithError(const std::string &errorString)
    {
        LuaStack* stack = LUAENGINE->getLuaStack();
        stack->pushString("onFailDeeplink");
        stack->pushString(errorString.c_str());
        stack->executeFunctionByHandler(m_luaHandler, 2);
    }
...
TOLUA_API int register_all_PluginTuneLua_helper(lua_State* L) {
